...
size_t platform_udpBroadcaster_write(platform_udpBroadcaster_t *this,void*data, size_t len)
{
	platform_assert(this);
	platform_assert(data);
	platform_assert(len);
	int res = sendto(this->base.sock, data, len,0, (struct sockaddr *)&this->broadcast_addr, sizeof(struct sockaddr_in));
	if (res < 0) {
...
size_t platform_udpBroadcaster_read(platform_udpBroadcaster_t *this, void*data, size_t len)
{
	platform_assert(this);
	platform_assert(data);
	platform_assert(len);
	...
	int res;
	while (res = recvfrom(this->base.sock, data, len, 0, (struct sockaddr *)&addr, &fromlen)) {
		if(res>0) {
...
bool platform_udpBroadcaster_init(platform_udpBroadcaster_t *this,uint16_t local_port,uint16_t remote_port)
{
	platform_assert(this);
...
#endif
    bOpt = true;
	int res= setsockopt(this->base.sock, SOL_SOCKET, SO_BROADCAST, &bOpt, sizeof(bOpt));
	if (res < 0) {
		if (this->base.sock < 0)
		{
			perror("socket failed.");
	...
	res = sendto(this->base.sock, get_local_ip, sizeof(get_local_ip), 0,(struct sockaddr *)&this->broadcast_addr, sizeof(struct sockaddr_in));
	if (res < 0)
	{
	...
	res = recvfrom(this->base.sock, get_local_ip, sizeof(get_local_ip), 0, (struct sockaddr *)&temp_addr, &addr_len);
...
platform_udpBroadcaster_t *platform_udpBroadcaster_new(uint16_t local_port, uint16_t remote_port)
{
	platform_udpBroadcaster_t *caster = (platform_udpBroadcaster_t *)malloc(sizeof(platform_udpBroadcaster_t));
	if (!caster)
		return NULL;
...
bool platform_udpMulticaster_init(platform_udpMulticaster_t *this,  uint16_t local_port, uint16_t remote_port)
{
	platform_assert(this);
	if (!platform_udpBase_init((platform_udpBase_t *)this, local_port))
		return false;
	...
	setsockopt(this->base.sock, IPPROTO_IP, IP_MULTICAST_LOOP, &loop, sizeof(loop));
	struct ip_mreq ipmr;
	ipmr.imr_interface.s_addr = htonl(INADDR_ANY);
	ipmr.imr_multiaddr.s_addr = inet_addr("234.5.6.7");
	setsockopt(this->base.sock, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char*)&ipmr, sizeof(ipmr));
	return true;
}

bool platform_tcpClient_init(platform_tcpClient_t *this,uint16_t port, unsigned long server_ip, uint16_t server_port)
{
	platform_assert(this);
	platform_assert(port>1024U);
	...
	if (connect(this->sock, (struct sockaddr *) &addr, sizeof(struct sockaddr_in)) < 0)
	{
		return false;
	}
	return true;
...
platform_tcpClient_t * platform_tcpClient_new( uint16_t port, unsigned long server_ip, uint16_t server_port)
{
	platform_tcpClient_t *client = (platform_tcpClient_t *)malloc(sizeof(platform_tcpClient_t));
	if (!client) {
		return NULL;
	}
	if (!platform_tcpClient_init(client, port, server_ip, server_port))
	{
		free(client);
		return	NULL;
	}
...
size_t platform_tcpClient_read(platform_tcpClient_t *this, void *buf, size_t len)
{
	platform_assert(this);
	platform_assert(buf);
	platform_assert(len);
...
size_t platform_tcpClient_write(platform_tcpClient_t *this, void *buf, size_t len)
{
	platform_assert(this);
	platform_assert(buf);
	platform_assert(len);
...
{
	platform_tcpServer_clientInfo_t *info = (platform_tcpServer_clientInfo_t *)arg;
...
void *platform_tcpServer_thread(void *arg)
{
	...
	while (1) {
		info = (platform_tcpServer_clientInfo_t *)malloc(sizeof(platform_tcpServer_clientInfo_t));
		if (!info)
		{
		...
		info->running = true;
		info->server = this;
		if (!platform_list_insert(this->connected_clients, NULL, info, sizeof(platform_tcpClient_t)))
		{
...
bool platform_tcpServer_init(platform_tcpServer_t *this,uint16_t port, platform_tcpServerCallback_t callback)
{
	platform_assert(this);
...
platform_tcpServer_t *platform_tcpServer_new(uint16_t port, platform_tcpServerCallback_t callback)
{
	platform_tcpServer_t *server = (platform_tcpServer_t *)malloc(sizeof(platform_tcpServer_t ));
	if (!server)
		return NULL;
