...
bscl_ftp_server_t *bscl_ftp_server_new(uint32_t ip, uint16_t control_port) {
  bscl_ftp_server_t *server = (bscl_ftp_server_t *)malloc(sizeof(bscl_ftp_server_t));
  if (!server) {
    return NULL;
  }
  server->cmd_fd = bscl_tcp_new(control_port);
  server->data_fd = bscl_tcp_new(0);
  server->client_num = 0;
  server->listen_ip = ip;
  return server;
}
int bscl_ftp_server_process(bscl_ftp_server_t *server, int client_fd) {
  ...
  while (1 == MAX_CMD_LEN - 1) {
    ...
    bscl_tcp_read(client_fd, cmd + i, 1);
    if (i > 0 && cmd[i] == '\n' && cmd[i - 1] == '\r') {
