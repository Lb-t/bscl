...
#define bscl_md5_STR_LEN 32

/* Constants for _bscl_md5_Transform routine. */
#define S11 7
#define S12 12
#define S13 17
#define S14 22
...
/* F, G, H and I are basic bscl_md5_ functions. */
...
/* bscl_md5_ initialization. Begins an bscl_md5_ operation, writing a new context. */
void bscl_md5_init(bscl_md5_context_t *context) {
...
/* Encodes input (unsigned int) into output (uint8_t). Assumes len is a multiple of 4. */
static void _Encode(uint8_t *output, unsigned int *input, unsigned int len) {
  unsigned int i, j;
...
/* Decodes input (uint8_t) into output (unsigned int). Assumes len is a multiple of 4.*/
static void _Decode(unsigned int *output, const uint8_t *input, unsigned int len) {
  unsigned int i, j;
