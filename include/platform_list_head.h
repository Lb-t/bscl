...
#define platform_list_first_entry(ptr, type, member) platform_list_head_data(type, member, (ptr)->next)

#define platform_list_for_each(pos, list) for (pos = (list)->next; pos != (list); pos = pos->next)

#define platform_list_for_each_prev(pos, list) for (pos = (list)->prev; pos != (list); pos = pos->prev)

#define platform_list_for_each_safe(pos, n, list) for (pos = (list)->next, n = pos->next; pos != (list); pos = n, n = pos->next)
...
#define platform_list_for_each_entry(pos, list, type, member)                                                                  \
  for ((pos) = platform_list_head_data(type, member, (list)->next); &(pos)->member != (list);                                  \
       (pos) = platform_list_head_data(type, member, (pos)->member.next))

#define platform_list_for_each_entry_safe(pos, n, list, type, member)                                                          \
  for (pos = platform_list_head_data(type, member, (list)->next), n = platform_list_head_data(type, member, pos->member.next); \
       &pos->member != (list); pos = n, n = platform_list_head_data(type, member, n->member.next))

#define platform_list_for_each_entry_reverse(pos, list, type, member)                                                          \
  for (pos = platform_list_head_data(type, member, (list)->prev); &pos->member != (list);                                      \
