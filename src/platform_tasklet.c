...
int platform_tasklet_link(struct platform_tasklet *tsl, struct platform_tasklet_head *th) {
  ...
  platform_list_insert_tail(&tsl->list, &th->list);
...
int platform_tasklet_is_linked(struct platform_tasklet *tsl) { return platform_list_is_empty(&tsl->list) ? 0 : 1; }
...
void platform_tasklet_unlink(struct platform_tasklet *tsl) {
  ...
  platform_list_head_remove(&tsl->list);
...
void platform_tasklet_pend(struct platform_tasklet *tsl) {
  if (tsl->pend == tsl->deal) {
    tsl->pend++;
    if (tsl->head->sem) {
      platform_os_sem_post(tsl->head->sem);
    }
  } else {
    tsl->pend++;
  }
...
void platform_tasklet_pend_isr(struct platform_tasklet *tsl) { platform_tasklet_pend(tsl); }

int platform_tasklet_is_pended(struct platform_tasklet *tsl) { return tsl->pend != tsl->deal ? 1 : 0; }

void platform_tasklet_clean_pend(struct platform_tasklet *tsl) { tsl->deal = tsl->pend; }
...
int platform_tasklet_dispatch(struct platform_tasklet_head *th, int timeout_ms) {
  ...
  platform_list_for_each_entry_safe(tsl, temp, &th->list, struct platform_tasklet, list) {
