...
#else
#define LOG(level, format, ...)                                                                    \
    fprintf(stdout, "%s %d:" #level " " format, __FILE__, __LINE__, ##__VA_ARGS__)
#endif
...
static int send_dirline(int s, int wide, time_t curTime, char const *path, char const *add,
                        char const *fname, char *buf)
{
    if (wide) {
        ...
        if (cb->stat(buf, &stat_buf) == 0) {
            ...
            len = snprintf(buf, FTPD_CMD_BUF_SIZE, "%crwxrwxrwx  1 0 0 %11u %s %s\r\n",
                           (stat_buf.type == OS_FS_TYPE_DIR) ? 'd' : '-', stat_buf.size, timeBuf,
                           fname);
...
static void command_pasv(ftpd_session_t *session)
{
    ...
    if (s < 0)
        ...
        if (0 > bind(s, (struct sockaddr *)&addr, addrLen))
            ...
            snprintf(buf, FTPD_CMD_BUF_SIZE, "Entering passive mode (%u,%u,%u,%u,%u,%u).", ip[0],
