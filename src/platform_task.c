...
jmp_buf jumper;
 ...
 struct platform_scheduler_t_{
  jmp_buf jumper;
  platform_list_head_t tasks_list;
};
...
void platform_scheduler_start(platform_scheduler_t *scheduler){
  ...
  while(1){
    ...
    platform_list_for_each_entry(task,&scheduler->tasks_list,platform_task_t,head)
      {
...
platform_scheduler_t* platform_scheduler_new(void){
  ...
  platform_list_init(&scheduler->tasks_list); 
  return scheduler;
}
...
int platform_task_start(platform_scheduler_t *scheduler,platform_task_t*task,void*arg){
  platform_list_insert_tail(&scheduler->tasks_list,&task->head) ;
  task->scheduler=scheduler;
  int ret=setjmp(task->jumper);
  if(ret==0){return 0;}
  else{
   ret= task->routine(task,arg);
   platform_list_head_remove(&task->head);
