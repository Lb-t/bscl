...
bool platform_list_insert(platform_list_t *list, platform_list_item_t *item, void *data, size_t len) {
  platform_assert(list);
  platform_list_item_t *newItem = (platform_list_item_t *)malloc(sizeof(platform_list_item_t));
  if (!newItem)
    return false;

  platform_os_mutex_lock(&list->mutex);
  if (NULL == item) {
  ...
  list->size += 1;
  platform_os_mutex_unlock(&list->mutex);
...
void platform_list_init(platform_list_t *list) {
  platform_assert(list);
  platform_os_mutex_create(&list->mutex);
  list->head = NULL;
  list->tail = NULL;
  list->size = 0;
}
...
void platform_list_deninit(platform_list_t *list) {
  ...
  platform_os_mutex_unlock(&list->mutex);
  platform_os_mutex_delete(&list->mutex);
}
