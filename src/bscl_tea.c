...
#include "bscl_tea.h"
static uint8_t bscl_tea_key[16] = { 0x29, 0x23, 0xbe, 0x84, 0xe1, 0x6c,
...
bool bscl_encryptData(uint8_t *data, uint32_t len) {
	...
	n = len / 8;
	bscl_encrypt((uint32_t *) (data + 8 * (n - 1)),
			(uint32_t *) bscl_tea_key);
	for (i = n - 2; i >= 0; i--) {
		bscl_xor_64(data + 8 * i, data + 8 * (i + 1));
		bscl_encrypt((uint32_t *) (data + 8 * i),
				(uint32_t *) bscl_tea_key);
	}
	return true;
}
...
bool bscl_decryptData(uint8_t *data, uint32_t len) {
	...
	for (i = 0; i < n - 1; i++) {
		bscl_decrypt((uint32_t *) (data + 8 * i),
				(uint32_t *) bscl_tea_key);
		bscl_xor_64(data + 8 * i, data + 8 * (i + 1));
	}
	bscl_decrypt((uint32_t *) (data + 8 * (n - 1)),
			(uint32_t *) bscl_tea_key);
	return true;
}
